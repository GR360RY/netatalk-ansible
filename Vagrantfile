...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define :ubuntu1204 do |ubuntu1204|
    ...
    ubuntu1204.vm.provision "ansible" do |ansible|
      ...
      ansible.extra_vars = { vm_hostname: "ubuntu1204" }
      ansible.sudo = true
    end
    ...
    ubuntu1204.vm.provider :virtualbox do |vbox|
      ...
      vbox.customize ["modifyvm", :id, "--name", 'ubuntu1204' ]
    end
  end

  config.vm.define :ubuntu1210 do |ubuntu1210|
    # Ubuntu Server 12.10 amd64 Minimal (VirtualBox 4.2.6) from http://www.vagrantbox.es/
    ubuntu1210.vm.provision "ansible" do |ansible|
      ansible.playbook = "netatalk.yml"
      ansible.extra_vars = { vm_hostname: "ubuntu1210" }
      ansible.sudo = true
    end
    ...
    ubuntu1210.vm.provider :virtualbox do |vbox|
      vbox.customize ['modifyvm', :id, '--memory', '1024']
      vbox.customize ["modifyvm", :id, "--name", 'ubuntu1210' ]
    end
  end
